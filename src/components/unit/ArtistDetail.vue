...
<template>
  <div class="px-5 pb-5` pc:px-14 pc:py-10">
    <div v-if="artist" class="max-w-4xl mx-auto space-y-6 pc:grid pc:mx-0 pc:grid-cols-[35%_65%] pc:w-full pc:max-w-full pc:gap-6 pc:space-y-0">
      <div class="space-y-3 pc:space-y-5">
        <div class="rounded-xl overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.12)]">
          <img
            ...
            class="w-full h-[450px] object-cover pc:h-[400px]"
          />
        </div>
        <div class="space-y-3">
          ...
          <div class="py-2">
            ...
            <div v-else class="text-sm">{{ t('artistDetail.noLabels') }}</div>
          </div>
        ...
        <div class="space-y-3 p-4 rounded-lg bg-[var(--bg)] shadow-[0_0_6px_var(--shadow-weak)] ">
          <div class="text-sm font-semibold">{{ t('artistDetail.links') }}</div>
          <div class="flex flex-wrap gap-3 justify-between">
            <a
              ...
              target="_blank"
              rel="noopener"
              class="inline-flex items-center justify-center w-8 h-8 rounded-md border border-[var(--stroke)] border-[var(--subline)] text-base text-[var(--text)] pc:hover:bg-[var(--surface)] transition-colors shadow-[1px_1px_3px_var(--shadow-weak)]"
            >
        ...
        <div class="space-y-3 p-4 rounded-lg bg-[var(--bg)] shadow-[0_0_6px_var(--shadow-weak)] ">
          <div class="text-sm font-semibold">{{ t('artistDetail.aliases') }}</div>
          <div class="flex flex-wrap gap-2">
            <span
              v-for="alias in (showAllAliases ? displayAliases : displayAliases.slice(0, 3))"
              :key="alias"
              class="px-3 py-1 rounded-md bg-[var(--subbg)] text-sm text-[var(--muted)]"
            >
              {{ alias }}
            </span>
      ...
      <div class="space-y-3 pc:space-y-5">
        <div class="space-y-3 p-4 rounded-lg bg-[var(--bg)] shadow-[0_0_6px_var(--shadow-weak)] ">
          <div class="text-sm font-bold">{{ t('artistDetail.latestFestival') }}</div>
          <template v-if="latestFestival">
            <router-link
              :to="{ name: 'festivaldetail', params: { id: latestFestival.id } }"
              class="flex items-center gap-3 p-3 rounded-lg bg-[var(--listbg)] pc:hover:bg-neonpink/10 transition-colors"
            >
              <img
              ...
              <div class="flex flex-col">
                ...
                <div class="text-xs text-[var(--muted)]">{{ latestFestival.city }} / {{ latestFestival.contry }}</div>
                <div class="text-xs text-[var(--muted)]">{{ latestFestival.start }} ~ {{ latestFestival.end }}</div>
              </div>
            </router-link>
          </template>
        ...
        <div class="p-4 rounded-lg bg-[var(--bg)] shadow-[0_0_6px_var(--shadow-weak)] space-y-4">
          ...
          <div v-if="displayAlbums.length">
            ...
            <div class="hidden pc:grid pc:grid-cols-3 pc:gap-4 pc:max-h-[450px] pc:overflow-y-auto pc:pr-1">
